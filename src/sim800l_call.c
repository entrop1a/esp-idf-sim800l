...
esp_err_t sim800l_call_make_call(sim800l_handle_t sim800l_handle, const char *number)
{
    ...
    /* Send AT command */
    esp_err_t ret = sim800l_out_data(sim800l_handle, command, (uint8_t *)"OK\r\n", 1000);
    if (ret != ESP_OK)
    {
...
esp_err_t sim800l_call_answer(sim800l_handle_t sim800l_handle, sim800l_call_response_t call_response)
{
    ...
    {
        /* Send AT command */
        ret = sim800l_out_data(sim800l_handle, (uint8_t *)SIM800L_COMMAND_CALL_HANGUP, (uint8_t *)"OK\r\n", 1000);
        if (ret != ESP_OK)
        {
    ...
    /* Send AT command */
    ret = sim800l_out_data(sim800l_handle, (uint8_t *)SIM800L_COMMAND_CALL_ANSWER, (uint8_t *)"OK\r\n", 1000);
    if (ret != ESP_OK)
    {
        ESP_LOGE(SIM800L_CALL_TAG, "sim800l_call_answer failed: %s", esp_err_to_name(ret));
        return ret;
...
esp_err_t sim800l_call_line_identify(sim800l_handle_t sim800l_handle, sim800l_call_line_id_t call_line_id)
{
    ...
    /* Send AT command */
    ret = sim800l_out_data(sim800l_handle, (uint8_t *)command, (uint8_t *)"OK\r\n", 1000);
    if (ret != ESP_OK)
    {
        ESP_LOGE(SIM800L_CALL_TAG, "sim800l_call_answer failed: %s", esp_err_to_name(ret));
        free(command);
